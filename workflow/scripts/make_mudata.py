...
def main():
    ...
    count_tsv = transcript_count_file.replace('discovered_transcript_grouped', 'transcript_grouped').replace('_counts.linear.tsv', '_counts.tsv')
    tpm_tsv = transcript_count_file.replace('discovered_transcript_grouped', 'transcript_grouped').replace('_counts.linear.tsv', '_tpm.tsv')
    ...
    reference_gene = None
    count_tsv = transcript_count_file.replace('discovered_transcript_grouped', 'gene_grouped').replace('_counts.linear.tsv', '_counts.tsv')
    tpm_tsv = transcript_count_file.replace('discovered_transcript_grouped', 'gene_grouped').replace('_counts.linear.tsv', '_tpm.tsv')
    if np.logical_and(os.path.exists(tpm_file) , os.path.exists(count_tsv)):
        adata = make_count_adata(count_tsv, tpm_tsv)
        adata = adata[mdata.obs_names,:]
        assert np.array_equal(adata.obs_names, mdata.obs_names)
