...
rule run_IsoQuant:
    ...
    shell: "python ./IsoQuant/isoquant.py --reference {params.fasta} --genedb {params.gtf} --bam {input} --data_type pacbio_css -p {config[project_name]} -o results/isoquant_output/ --read_group tag:SM --complete_genedb --count_exons"
...
rule make_MuData:
    ...
    conda: "requirements.yaml"
    shell: "python ./make_mudata.py --counts {input.counts} --models {input.models} --output-dir results/isoquant_output/mudata/"
