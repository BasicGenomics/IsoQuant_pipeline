...
def plot_pie_assignment(
    ...
    if save:
        if os.path.exists(obj.plot_output) == False:
            obj.plot_output = f'{obj.output_directory}/plot_output'
            os.makedirs(obj.plot_output,exist_ok=True)
        out_dir = obj.plot_output
        ofname = os.path.join(out_dir,f'Pie_{feature_to_plot}.png')
    ...
    if is_nonempty(gene_list):
        ...
        for g in gene_list:
            ...
            if save:
                if os.path.exists(obj.plot_output) == False:
                    ...
                    os.makedirs(obj.plot_output,exist_ok=True)
                out_dir = obj.plot_output
                ofname = os.path.join(out_dir,f'Bar_{g}.png')
                plt.savefig(ofname, format="png", dpi=144, bbox_inches='tight')
        ...
        if save:
            if os.path.exists(obj.plot_output) == False:
                ...
                os.makedirs(obj.plot_output,exist_ok=True)
            out_dir = obj.plot_output
            ofname = os.path.join(out_dir,f'Bar_isoforms.png')
            plt.savefig(ofname, format="png", dpi=144, bbox_inches='tight')
...
def plot_transcript_map(obj,
        ...
        if save:
            ...
            out_dir = obj.plot_output
            if use_transcript_model:
                ofname = os.path.join(out_dir,f'TranscriptMap_wModel.png')
            else: 
                ofname = os.path.join(out_dir,f'TranscriptMap.png')
            plt.savefig(ofname, format="png", dpi=144, bbox_inches='tight')
...
# ---  fix ticks and others  ---
    ...
    if save:
        if os.path.exists(obj.plot_output) == False:
            ...
            os.makedirs(obj.plot_output,exist_ok=True)
        out_dir = obj.plot_output
        ofname = os.path.join(out_dir,f'GenomeTrack_{region}.png')
        plt.savefig(ofname, format="png", dpi=144, bbox_inches='tight')
